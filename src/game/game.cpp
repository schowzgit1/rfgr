...
void FiveM::ESP::RunESP(UIBase& ui)
{
	...
	for (int i = 0; i < 200; i++) {
		...
		if (allVisible) {
			...
			// Draw armor bar
			float armor = mem.Read<float>(ped + 0x150C);
			float armorPercentage = armor / 100.0f;
			float armorBarWidth = 4.0f;
			float armorBarHeight = boxHeight;
			float armorBarX = healthBarX - armorBarWidth - 4.0f;
			float armorBarY = boxY;

			// Background (dark blue)
			draw_list->AddRectFilled(
				ImVec2(armorBarX, armorBarY),
				ImVec2(armorBarX + armorBarWidth, armorBarY + armorBarHeight),
				ImColor(0, 0, 100, 255)
			);

			// Armor (light blue)
			float armorBarFillHeight = armorBarHeight * armorPercentage;
			draw_list->AddRectFilled(
				ImVec2(armorBarX, armorBarY + armorBarHeight - armorBarFillHeight),
				ImVec2(armorBarX + armorBarWidth, armorBarY + armorBarHeight),
				ImColor(0, 150, 255, 255)
			);

			// Add armor text
			char armorText[8];
			sprintf_s(armorText, "%.0f", armor);
			textWidth = ImGui::CalcTextSize(armorText).x;
			textX = armorBarX - textWidth - 4.0f;
			textY = armorBarY + armorBarHeight - armorBarFillHeight - 8.0f;
			draw_list->AddText(ImVec2(textX, textY), ImColor(255, 255, 255, 255), armorText);
		}
	}
}
